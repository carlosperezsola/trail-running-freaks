...
use App\DataTables\ThirdPartyProductOptionItemDataTable;
...
class ThirdPartyProductOptionItemController extends Controller
{
    public function index(ThirdPartyProductOptionItemDataTable $dataTable, $productId, $optionId)
    {
        $product = Product::findOrFail($productId);

        $option = ProductOption::findOrFail($optionId);
        return $dataTable->render('third_party_user.product.product-option-item.index', compact('product', 'option'));
    }
    ...
    public function create(string $productId, string $optionId)
    {
        $option = ProductOption::findOrFail($optionId);
        $product = Product::findOrFail($productId);
        return view('third_party_user.product.product-option-item.create', compact('option', 'product'));
    }
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('third_party_user.products-option-item.index',
    ...
    public function edit(string $optionItemId)
    {
        ...
        return view('third_party_user.product.product-option-item.edit', compact('optionItem'));
    }
